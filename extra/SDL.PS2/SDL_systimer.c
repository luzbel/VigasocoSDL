...
struct str_sleeping_threads { int  id ; Uint32 ticks; };
static struct str_sleeping_threads sleeping_threads[MAX_SLEEPING_THREADS];
...
void SDL_Delay(Uint32 ms)
{
	...
	sleeping_threads[i].id = GetThreadId();
	sleeping_threads[i].ticks = SDL_GetTicks()+ms;

	/* pruebas */
//Uint32 ticks=SDL_GetTicks()+ms;
	while(sleeping_threads[i].ticks>SDL_GetTicks())
	{
		SleepThread();
	}
	

	/* Esto falla si se le da prioridad al hilo por otro lado, y no solo lo llama el ms_handler
	   ya que decrementa ms siempre que se devuelve al control al hilo y no solo cuando
	   ha pasado 1ms 
	while (ms > 0)
	{
		SleepThread();
	...
	nsleeping_threads--;
	sleeping_threads[i].id = sleeping_threads[nsleeping_threads].id;
	sleeping_threads[i].ticks = sleeping_threads[nsleeping_threads].ticks;
	sleeping_threads[nsleeping_threads].id = -1;
}
...
static int ms_handler(int ca)
{
	...
	{
		if (sleeping_threads[i].id >= 0)
		{
			iWakeupThread(sleeping_threads[i].id);
		}
	}
